...
async function cargarDatosFirestore() {
    ...
    await Promise.all(directorQuerySnapshot.docs.map(async (directorDoc) => {
        ...
        const storageRef = firebase.storage().ref().child('tu_carpeta');

        // Lista todos los elementos (archivos y subdirectorios) en la carpeta
        storageRef.listAll().then((result) => {
            result.items.forEach((item) => {
                // Accede al nombre del archivo
                const nombreArchivo = item.name;

                // Ahora puedes usar el nombre del archivo para mostrarlo o realizar otras operaciones
                console.log('Nombre del archivo:', nombreArchivo);

                // Por ejemplo, si deseas mostrar una lista de enlaces a los archivos
                const enlace = document.createElement('a');
                enlace.href = item.fullPath; // Usar fullPath para el enlace
                enlace.innerText = nombreArchivo;
        ...
        if (asesorQuerySnapshot.empty) {
            ...
            await Promise.all(asesorQuerySnapshot.docs.map(async (asesorDoc) => {
                ...
                const carpetaPagoAsesor = firebase.storage().ref().child('users/' + idAsesor + '/comprobantes-pago/');
                  ...
                  result.items.forEach((itemRef) => {
                    ...
                    itemRef.getDownloadURL()
                      .then((url) => {
                        ...
                        console.log('Nombre del archivo:', nombreArchivo);
                        console.log('Enlace de descarga:', url);
                ...
                const DocumentoAsesor = asesorData.Documento,
                    ...
                    MesPagoAsesor = parseInt(directorData.MesPago);
                const fechaActual = new Date();
                const mesActual = (fechaActual.getMonth() + 1);
                const ArchivoComprobantePagoAsesor = mesActual !== MesPagoAsesor ? false : directorData.ArchivoComprobantePago;
                const EstadoComprobantePagoAsesor = mesActual !== MesPagoAsesor ? false : directorData.EstadoComprobantePago;
                ...
                const asesorItem = { NombreAsesor: NombreAsesor, Archivo_DocumentoAsesor: Archivo_DocumentoAsesor, CompraventaAsesor: CompraventaAsesor, CuentasParticipacionAsesor: CuentasParticipacionAsesor, AutorizacionMejorasAsesor: AutorizacionMejorasAsesor, completadoAsesor: completadoAsesor, UltimoPagoAsesor: UltimoPagoAsesor };
                ...
                if (inversorQuerySnapshot.empty) { const objetoCombinado = Object.assign({}, directorItem, asesorItem); rowData.push(objetoCombinado); } else {
                    await Promise.all(inversorQuerySnapshot.docs.map(async (inversorDoc) => {
                        ...
                        const NombreInversor = inversorData.Nombre + " " + inversorData.Apellido,
                            ...
                            MesPagoInversor = parseInt(directorData.MesPago);
                        const fechaActual = new Date();
                        const mesActual = (fechaActual.getMonth() + 1);
                        const ArchivoComprobantePagoInversor = mesActual !== MesPagoInversor ? false : directorData.ArchivoComprobantePago;
                        const EstadoComprobantePagoInversor = mesActual !== MesPagoInversor ? false : directorData.EstadoComprobantePago;
