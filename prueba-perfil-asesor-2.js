...
auth.onAuthStateChanged((user) => {
    if (user) {
        ...
        userDocumentRef.get().then((doc) => {
            ...
            $("#estado-contrato-director").text(datos.EstadoContratoDirector ? datos.EstadoContratoDirector : "En espera de archivo");
...
function barra() {
    ...
    userDocumentRef.get().then((doc) => {
        const datos = doc.data();
        const estados = [datos.EstadoArchivoCompraVenta, datos.EstadoCuentasParticipacion, datos.EstadoAutorizacionMejoras, datos.EstadoComprobantePago], totalEstados = estados.length, estadosDefinidos = estados.filter(o => null != o), cantidadEstadosDefinidos = estadosDefinidos.length, porcentajeEstadosDefinidos = cantidadEstadosDefinidos / totalEstados * 100; cantidadEstadosDefinidos > 0 && ($("#barra-informacion").css("width", porcentajeEstadosDefinidos + "%"), $("#porcentaje_informacion").text(Math.round(porcentajeEstadosDefinidos) + "%"));
    });
}
...
const cargarContratoDirector = document.getElementById("cargar-contrato-director");
const cargarContratoDirectorInput = document.createElement("input");
cargarContratoDirectorInput.setAttribute("type", "file");
cargarContratoDirectorInput.setAttribute("id", "cargar-archivo-contrato-director");
cargarContratoDirectorInput.setAttribute("accept", ".pdf");
cargarContratoDirectorInput.style.display = "none";

cargarContratoDirector.parentNode.insertBefore(cargarContratoDirectorInput, cargarContratoDirector.nextSibling);

cargarContratoDirector.addEventListener("click", () => {
    cargarContratoDirectorInput.click();
});

document.addEventListener("DOMContentLoaded", function () {
    const inputContratoDirector = document.getElementById("cargar-archivo-contrato-director");

    inputContratoDirector.addEventListener("change", e => {
        $("#texto-contrato-director").text("Cargando archivo...");
        let file = e.target.files[0];
        let storageRef = firebase.storage().ref(`users/${userId}/contrato-director`);

        storageRef.put(file).then(() => {
            storageRef.getDownloadURL().then(url => {
                let userRef = db.collection("users").doc(userId);
                userRef.update({ ArchivoContratoDirector: url, EstadoContratoDirector: "En revisión" }).then(() => {
                    $("#estado-contrato-director").text("En revisión");
                    $("#texto-contrato-director").text("Cargar");
                    barra();
                }).catch(error => {
                    $("#texto-contrato-director").text("Error al cargar archivo");
                    console.error(error);
                });
            });
        });
