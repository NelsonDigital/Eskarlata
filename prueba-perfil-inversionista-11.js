...
function barra() {
    ...
    userDocumentRef.get().then((doc) => {
        ...
        const estados = [datos.EstadoArchivoCompraVenta, datos.EstadoCuentasParticipacion, datos.EstadoAutorizacionMejoras, datos.EstadoComprobantePago, datos.EstadoDocumento],
            totalEstados = estados.length,
            estadosDefinidos = estados.filter(o => o !== null && o !== 'En espera de archivo' && o !== 'Denegado'),
            cantidadEstadosDefinidos = estadosDefinidos.length, porcentajeEstadosDefinidos = cantidadEstadosDefinidos / totalEstados * 100;
        cantidadEstadosDefinidos > 0 && ($("#barra-informacion").css("width", porcentajeEstadosDefinidos + "%"),
            $("#porcentaje_informacion").text(porcentajeEstadosDefinidos + "%"));
    });
}
