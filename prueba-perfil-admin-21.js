...
async function cargarDatosFirestore() {
    const rowData = [];
    const directorQuerySnapshot = await collectionRef.where("Rol", "==", "Director").where("habilitado", "!=", "false").get();
    await Promise.all(directorQuerySnapshot.docs.map(async (directorDoc) => {
        ...
        const asesorQuerySnapshot = await collectionRef.where("documentoreferencia", "==", Documento).where("habilitado", "!=", "false").get();
        if (asesorQuerySnapshot.empty) {
            ...
            await Promise.all(asesorQuerySnapshot.docs.map(async (asesorDoc) => {
                ...
                const inversorQuerySnapshot = await collectionRef.where("documentoreferencia", "==", DocumentoAsesor).where("habilitado", "!=", "false").get();
