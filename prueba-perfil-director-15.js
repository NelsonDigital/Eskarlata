...
auth.onAuthStateChanged((user) => {
    if (user) {
        ...
        userDocumentRef.get().then((doc) => {
            ...
            const fechaObjetivo = new Date(2030, 8, 1); 
            const abonos = (fechaObjetivo.getFullYear() - fecha_creacion.getFullYear()) * 12 + fechaObjetivo.getMonth() - fecha_creacion.getMonth();
            $("#abonos").text(abonos);
            console.log("Fecha creacion: "+fecha_creacion+"Fecha objetivo: "+fechaObjetivo+"abonos: "+abonos);
...
function barra() {
    ...
    userDocumentRef.get().then((doc) => {
        ...
        $("#estado-comprobante-pago").text(datos.EstadoComprobantePago);
        $("#estado-documento").text(datos.EstadoDocumento ? datos.EstadoDocumento : "En espera de archivo");

        const estados = [datos.EstadoContratoOjo, datos.EstadoArchivoCompraVenta, datos.EstadoCuentasParticipacion, datos.EstadoArchivoAutorizacionMejoras, datos.EstadoComprobantePago, datos.EstadoDocumento], totalEstados = estados.length, estadosDefinidos = estados.filter(o => null != o), cantidadEstadosDefinidos = estadosDefinidos.length, porcentajeEstadosDefinidos = cantidadEstadosDefinidos / totalEstados * 100; cantidadEstadosDefinidos > 0 && ($("#barra-informacion").css("width", porcentajeEstadosDefinidos + "%"), $("#porcentaje_informacion").text(Math.round(porcentajeEstadosDefinidos) + "%"));
    });
}
